...
WorldIndex build_world_index(const World& world, const std::string& source) {
    ...
    const WorldZoneGraph graph = build_world_zone_graph(normalized);
...
const WorldIndexZone* find_world_index_zone(const WorldIndex& index, const int zone_num) {
    for (const auto& zone : index.zones) {
...
bool reservation_range_conflicts(const WorldIndex& index, const WorldIndexReservation& reservation) {
    for (long vnum = reservation.start_vnum; vnum <= reservation.end_vnum; ++vnum) {
        if (reservation.kind == "room" && room_vnum_taken(index, vnum)) {
            return true;
        }
        if (reservation.kind == "mob" && mob_vnum_taken(index, vnum)) {
            return true;
        }
        if (reservation.kind == "object" && object_vnum_taken(index, vnum)) {
            return true;
        }
    }
    return false;
...
std::optional<long> suggest_room_vnum_in_zone(const WorldIndex& index, const int zone_num) {
...
std::optional<VnumRange> suggest_room_vnum_range_in_zone(const WorldIndex& index, const int zone_num,
    ...
    const WorldIndexZone* zone = find_world_index_zone(index, zone_num);
    if (!zone || block_size <= 0) {
        return std::nullopt;
    }

    for (const auto& range : zone->rooms_free) {
        for (long start = range.start; start <= range.end; ++start) {
            if (room_vnum_taken(index, start)) {
                continue;
            }
            long end = start;
            for (int count = 1; count < block_size && end < range.end; ++count) {
                const long next = end + 1;
                if (room_vnum_taken(index, next)) {
                    break;
                }
                end = next;
            }
            bool ok = true;
            for (long vnum = start; vnum <= end; ++vnum) {
                if (room_vnum_taken(index, vnum)) {
                    ok = false;
                    break;
                }
...
bool validate_reservation(const WorldIndex& index, const WorldIndexReservation& reservation,
    ...
    if (reservation.builder.empty()) {
        return fail("builder is required");
    }
    if (reservation.zone_num <= 0) {
        return fail("zone_num is required");
    }
    if (reservation.end_vnum < reservation.start_vnum) {
        return fail("end_vnum must be >= start_vnum");
    }

    const WorldIndexZone* zone = find_world_index_zone(index, reservation.zone_num);
    if (!zone) {
        return fail("zone #" + std::to_string(reservation.zone_num) + " not found in world index");
    }

    if (reservation.kind == "room") {
        if (reservation.start_vnum < zone->bottom || reservation.end_vnum > zone->top) {
            return fail("room vnum range must be within zone bottom-top ["
                        + std::to_string(zone->bottom) + "-" + std::to_string(zone->top) + "]");
        }
    } else if (reservation.start_vnum <= 0 || reservation.end_vnum <= 0) {
        return fail("start_vnum and end_vnum are required");
    }

    if (reservation_range_conflicts(index, reservation)) {
        return fail("requested vnum range overlaps used or reserved vnums");
    }
